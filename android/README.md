...
운영 사이트의 자체 회원가입·ID/PW 로그인을 사용합니다. Vercel 계정은 필요하지 않습니다. 자동로그인을 선택하면 해당 기기의 브라우저에서 30일간 유지됩니다. APK에 DB 주소, DB 토큰, Vercel 인증 우회 키 또는 사용자 로그인 정보를 포함하지 않습니다. 재고 데이터는 웹사이트에서 처리합니다. 웹사이트의 수정은 별도 APK 업데이트 없이 반영됩니다. 1.1.1부터 뒤로가기 시 보이는 앱 시작 화면도 로그인 필요 및 자동로그인 설정 안내로 변경했습니다. 이 시작 화면의 문구는 APK에 포함되므로 기존 앱은 업데이트 설치가 필요합니다.
...
결과는 `work/android-output/jeago-1.1.1.apk`입니다. 1.0.0, 1.0.1, 1.1.0과 같은 서명 키를 사용하므로 기존 앱 위에 업데이트로 설치할 수 있습니다. APK 서명, 정렬, 패키지·버전·SDK 메타데이터를 빌드 스크립트에서 검증합니다. 실제 휴대폰 설치·실행 확인은 별도로 필요합니다.
