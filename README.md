...
로그인 후 상단의 **내 계정**(`/account`)에서 ID, 이름, 이메일, 연락처, 부서를 수정할 수 있습니다. 이메일·연락처·부서는 선택 항목이며 이메일은 인증 또는 비밀번호 복구 수단이 아닙니다. 계정 정보 저장과 비밀번호 변경 모두 현재 비밀번호 확인이 필요합니다. 이름 변경은 이후 입출고 담당자에 반영되고 과거 입출고 이력의 이름은 보존됩니다.

비밀번호는 10~128자로 설정하며 변경 시 모든 기기의 기존 세션을 종료합니다. 새 비밀번호로 다시 로그인한 뒤 자동로그인을 선택할 수 있습니다. 계정 수정은 요청의 사용자 ID가 아닌 로그인 세션의 계정에만 적용하며 변경 시 세션과 기존 비밀번호 해시를 트랜잭션에서 재확인합니다. 기존 회원 테이블에는 이메일·연락처·부서 열만 기본값이 빈 문자열인 상태로 추가합니다.

모바일·태블릿(너비 1150px 이하)에서는 재고 현황의 카테고리를 기본적으로 접습니다. **카테고리 선택**을 눌러 펼치고 항목을 선택하면 자동으로 접힙니다. 펼친 목록은 화면 높이의 45%, 최대 360px 안에서 스크롤되어 재고 목록을 길게 밀어내지 않습니다. PC의 왼쪽 카테고리 목록은 유지합니다. 이 웹 기능은 APK 1.2.0 재설치 없이 앱 재실행 또는 페이지 새로고침으로 반영됩니다.
