...
휴지통은 **복구**, 항목별 **영구 삭제**, **전체 비우기**를 제공합니다. 영구 삭제는 확인창에서 승인한 경우에만 진행되며 포함된 모델의 입출고 이력도 함께 삭제합니다. 거래처만 영구 삭제하면 과거 출고 이력의 업체명은 유지합니다. 다른 휴지통 그룹에 하위 모델/카테고리가 남아 있으면 먼저 하위 항목을 삭제하거나 전체 비우기를 사용하세요. 전체 비우기 직전에 목록이 바뀌었다면 재확인을 요청합니다. 삭제 후 정수 ID를 재사용하지 않아 오래된 화면이 새 자료를 수정하는 것을 방지합니다.

출고가 성공하면 입력한 업체명을 거래처 관리에 자동 등록합니다. 이름 앞뒤 공백, 연속 공백, 영문 대소문자를 정규화하여 중복을 검사하며 실패한 출고와 재시도는 중복 거래처를 만들지 않습니다. 기존 출고 이력의 업체도 최초 배포 후 한 번 반영합니다. 이미 휴지통으로 보낸 거래처는 이 초기 반영에서 제외합니다. 수동 등록/수정/복구에서도 같은 이름의 활성 거래처를 중복 등록할 수 없습니다.

출고업체 입력란은 활성 거래처와 남아 있는 출고 이력에서 이름을 추천합니다. 입력한 이름으로 시작하는 항목만 표시하므로 `베스`는 `베스트시스템`/`베스트아이티`를, `베스트시`는 `베스트시스템`만 표시합니다. 목록은 터치·클릭 또는 방향키와 Enter로 선택할 수 있고 한글 조합 중 Enter는 선택으로 처리하지 않습니다. 추천 목록에 없는 이름도 직접 입력할 수 있습니다. APK를 재설치할 필요 없이 웹 변경이 반영됩니다.

## 내 계정 및 모바일 카테고리

상단의 재고 관리 제목과 PC 왼쪽 로고는 첫 화면을 새로 불러오는 홈 링크입니다. 첫 화면에는 전체 재고 목록을 펼치지 않으며 카테고리·검색·요약 필터를 선택하면 목록을 표시합니다. 카테고리의 전체 모델을 선택하면 전체 목록을 조회할 수 있습니다.

새 모델 등록의 담당자는 로그인 계정의 이름으로 자동 표시되며 서버에서도 로그인 세션의 이름으로 저장합니다. 기존 모델 수정 시에는 저장된 담당자를 유지합니다. 고정 담당자 목록에 없는 회원 이름도 기존 값 그대로 저장할 수 있습니다.
...
비밀번호는 4~128자로 설정하며 변경 시 모든 기기의 기존 세션을 종료합니다. 새 비밀번호로 다시 로그인한 뒤 자동로그인을 선택할 수 있습니다. 계정 수정은 요청의 사용자 ID가 아닌 로그인 세션의 계정에만 적용하며 변경 시 세션과 기존 비밀번호 해시를 트랜잭션에서 재확인합니다. 기존 회원 테이블에는 이메일·연락처·부서 열만 기본값이 빈 문자열인 상태로 추가합니다.
...
- 메인 `/`에서 로그인하고 `/signup`에서 회원가입합니다. 가입 회원은 모두 같은 재고를 함께 관리합니다. 회원 정보 관리는 지정된 관리자만 이용할 수 있습니다.
...
- **자동로그인**: 선택한 기기의 서버 세션은 기간 만료 없이 유지하며 앱 사용 시 인증 쿠키를 갱신합니다. 유효한 기존 30일 자동로그인도 접속 시 전환합니다. 자동로그인 미선택 로그인은 기존 12시간 제한을 유지합니다. 로그아웃·비밀번호 변경·기기 데이터 삭제 시에는 다시 로그인해야 합니다. 브라우저 쿠키는 최대 400일로 발급하고 사용 중 갱신하므로, 장기간 접속하지 않거나 브라우저가 쿠키를 제거하면 재로그인이 필요할 수 있습니다. [Chrome 쿠키 보존 제한](https://developer.chrome.com/blog/cookie-max-age-expires/).
...
재고 화면의 묶음 출고에서 모델을 검색·선택하면 각각 1개로 설정됩니다. 모델별 수량을 조정하고 거래처를 한 번 입력해 출고합니다. 담당자는 로그인 계정으로 기록하며 거래처 자동 등록·추천을 재사용합니다. 최대 100개 모델의 재고와 버전을 한 트랜잭션에서 검증하고 한 항목이라도 실패하면 모두 취소합니다. 출고 이력은 모델별로 생성되며 동일 요청 재시도는 중복 차감하지 않습니다.

## 연차계산

상단 연차계산 버튼은 /leave로 이동합니다. 기존 모델 등록 메뉴는 유지합니다. 로그인한 모든 회원은 직원의 입사일·직급·특별연차와 사용 내역을 조회·수정할 수 있습니다. 날짜는 달력 입력이며 기간 선택 시 주말을 제외한 일수를 제안하고 공휴일은 직접 조정합니다. 반차는 0.5일입니다. 내역 추가·수정 후 연차 내역 저장을 누릅니다.

계산은 제공된 연차계산기.xlsx의 D3:E7 수식을 그대로 구현합니다. 1년 미만은 완료 개월 수, 이후는 매년 15+INT((연차-1)/2)의 누적 합과 남은 개월 비례분의 정수 부분, 특별연차를 합산합니다. 사용 내역은 엑셀과 동일하게 날짜와 무관하게 전부 합산하며 예약분도 포함합니다. 법정 연차 정책으로 치환하거나 소멸·상한을 새로 적용하지 않습니다. 직원 정보는 leave_employee에 저장하고 버전 충돌·요청 ID로 동시 수정과 중복 저장을 방지합니다.

원본 개인 자료는 저장소에 포함하지 않습니다. 검증된 비공개 JSON은 scripts/import-leave.mjs에 JSON 경로와 환경 파일 경로를 인자로 전달해 한 번 이전하며, 기존 자료는 덮어쓰지 않습니다.
