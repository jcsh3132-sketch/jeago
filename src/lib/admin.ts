...
export async function editMember(token: string | undefined, action: 'admin-profile' | 'admin-password' | 'admin-transfer', fields: Record<string, unknown>) {
  ...
  try {
    ...
    } else if (profile) await tx.execute({ sql: 'UPDATE app_user SET username=?,display_name=?,email=?,phone=?,department=?,account_version=account_version+1 WHERE id=?', args: [profile.username, profile.display_name, profile.email, profile.phone, profile.department, target] });
    else {
