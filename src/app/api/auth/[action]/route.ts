...
import { AuthError, authenticate, credentials, createSession, limitAuth, register, revokeSession, REMEMBER_SECONDS, SESSION_COOKIE, secureCookies } from '@/lib/auth';
export const runtime = 'nodejs';
export async function POST(request: NextRequest, context: { params: Promise<{ action: string }> }) {
  const { action } = await context.params;
  if (!['login', 'signup', 'logout'].includes(action)) return NextResponse.json({ message: '지원하지 않는 요청입니다.' }, { status: 404 });
