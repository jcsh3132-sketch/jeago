...
const nav = [['/', '▦', '재고 현황'], ['/add', '＋', '모델 등록'], ['/transactions', '↻', '입출고 내역'], ['/categories', '≡', '카테고리 관리'], ['/partners', '◎', '거래처 관리']];
export function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    <section className="app-main"><header className="topbar"><div><p className="eyebrow">INVENTORY MANAGEMENT</p><h1>{title}</h1></div><div className="top-actions"><Link href="/" className="top-btn ghost">재고 목록</Link><Link href="/add" className="top-btn primary">+ 모델 추가</Link></div></header><main className="content-area">{children}</main></section>
  </div>;
}
