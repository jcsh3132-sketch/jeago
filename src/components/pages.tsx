...
export function StockEditor({ data, item, outbound }: { data: InventoryData; item: Item; outbound: boolean }) {
  return <div className="form-page"><section className="form-card transaction-card"><div className="form-heading"><span className="section-kicker">{outbound ? 'OUTBOUND' : 'INBOUND'}</span><h2>{item.name}</h2><p>현재 재고 <strong>{item.quantity}개</strong> · {categoryPath(data.categories, item.category)}</p></div>
    <ActionForm action={outbound ? 'stock.out' : 'stock.in'}><Hidden name="id" value={item.id}/><label className="field field-label">{outbound ? '출고' : '입고'} 수량<input name="quantity" type="number" min="1" max={outbound ? item.quantity : 2147483647} step="1" required autoFocus/></label><ManagerSelect value={item.manager}/>
...
export function Categories({ data }: { data: InventoryData }) {
  ...
  function branch(c: Category, level: number) {
    ...
    return <details className={`admin-node level-${level}`} key={c.id} open={level === 0}><summary><strong>{label(c)}</strong><span className="category-count">{level === 2 ? `모델 ${items.length}개` : `${level === 0 ? '장비' : '품목'} ${children.length}개`}</span></summary>
      <div className="admin-node-body"><div className="admin-tools"><ActionForm action="category.rename" key={`${c.id}-${label(c)}`}><Hidden name="id" value={c.id}/><input name="name" aria-label={`${label(c)} 새 이름`} defaultValue={label(c)} required maxLength={80}/><button className="top-btn ghost">이름 변경</button></ActionForm>
      {level > 0 && <ActionForm action="category.move"><Hidden name="id" value={c.id}/><select key={c.parent_id} name="parent_id" defaultValue={c.parent_id || ''} aria-label={`${label(c)} 이동 대상`} required>{targets.map(t => <option key={t.id} value={t.id}>{level === 2 ? `${label(mains.find(m => m.id === t.parent_id))} · ` : ''}{label(t)}</option>)}</select><button className="top-btn ghost">이동</button></ActionForm>}
      <DeleteButton action="category.delete" id={c.id} message={level === 0 ? `${label(c)} 대분류를 삭제하시겠습니까? 장비가 포함된 대분류는 삭제할 수 없습니다.` : `${label(c)} 및 하위 모델과 모든 입출고 이력이 함께 삭제됩니다. 계속하시겠습니까?`}/></div>
      {children.map(n => branch(n, level + 1))}{level === 2 && <div className="admin-items">{items.map(i => <div key={i.id}><Link href={`/?category=${encodeURIComponent(i.category)}&item=${i.id}#item-${i.id}`}>{i.name}</Link><small>재고 {i.quantity}</small><Link href={`/edit/${i.id}`}>수정</Link><DeleteButton action="item.delete" id={i.id} message={`${i.name} 및 입출고 이력을 삭제하시겠습니까?`}/></div>)}{!items.length && <p className="muted">등록된 모델이 없습니다.</p>}</div>}</div>
    </details>;
  }
